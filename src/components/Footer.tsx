import Link from "next/link";

const IconFacebook = () => (
  <svg
    fill="currentColor"
    className="h-6 w-6"
    viewBox="0 0 24 24"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.772-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" />
  </svg>
);

const IconInstagram = () => (
  <svg
    fill="currentColor"
    className="h-6 w-6"
    viewBox="0 0 24 24"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.85s-.011 3.584-.069 4.85c-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07s-3.584-.012-4.85-.07c-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.069-1.644-.069-4.849s.011-3.584.069-4.85c.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.644-.069 4.85-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12s.014 3.667.072 4.948c.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24s3.667-.014 4.948-.072c4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948s-.014-3.667-.072-4.948c-.196-4.354-2.617-6.78-6.979-6.98C15.667.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.88 1.44 1.44 0 000-2.88z" />
  </svg>
);

const IconGitHub = () => (
  <svg
    fill="currentColor"
    className="h-6 w-6"
    viewBox="0 0 24 24"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.21 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12" />
  </svg>
);

export default function Footer() {
  return (
    <footer className="mt-12 border-t bg-gray-900 p-8 text-gray-400">
      <div className="mx-auto max-w-7xl">
        <div className="grid grid-cols-1 gap-8 text-center lg:grid-cols-3 lg:text-left">
          <div>
            <h3 className="text-2xl font-bold text-white">NexoPixel</h3>
          </div>

          <div>
            <h3 className="text-lg font-semibold uppercase text-white">
              Sobre Nós
            </h3>
            <p className="mt-2 max-w-xs mx-auto lg:mx-0">
              Seu hub de notícias sobre games, animes, cinema e tv, gerado por
              IA e curado por humanos.
            </p>
          </div>

          <div>
            <h3 className="text-lg font-semibold uppercase text-white">
              Siga-nos
            </h3>
            <div className="mt-4 flex space-x-4 justify-center lg:justify-start">
              <a
                href="#"
                className="rounded-md bg-gray-800 p-2 text-white hover:bg-gray-700"
                aria-label="Facebook"
              >
                <IconFacebook />
              </a>
              <a
                href="#"
                className="rounded-md bg-gray-800 p-2 text-white hover:bg-gray-700"
                aria-label="Instagram"
              >
                <IconInstagram />
              </a>
              <a
                href="https://github.com/gfranca91/nexopixel"
                className="rounded-md bg-gray-800 p-2 text-white hover:bg-gray-700"
                aria-label="GitHub"
                target="_blank"
                rel="noopener noreferrer"
              >
                <IconGitHub />
              </a>
            </div>
          </div>
        </div>

        <div className="mt-8 border-t border-gray-800 pt-4 text-center text-sm">
          <p>
            © {new Date().getFullYear()} NexoPixel. Todos os direitos
            reservados.
          </p>
        </div>
      </div>
    </footer>
  );
}
